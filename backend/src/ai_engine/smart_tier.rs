...
use arrow_array::{RecordBatch, RecordBatchIterator, ArrayRef, StringArray, Float32Array, FixedSizeListArray};
...
impl SmartTierEngine {
    ...
    pub async fn index_workspace(&self, path: &str) -> Result<usize> {
        ...
        for entry in walkdir::WalkDir::new(path) {
            ...
            if entry.file_type().is_file() {
                if let Some(ext) = entry.path().extension() {
                    ...
                    if ["rs", "swift", "py", "js", "ts", "cpp", "c", "h", "java", "kt"].contains(&ext.as_ref()) {
        ...
        let vector_array = FixedSizeListArray::try_new(Arc::new(vector_field), 384, Arc::new(vector_values), None)?;
    ...
    async fn get_or_create_table(&self) -> Result<Table> {
         if self.db.open_table(&self.table_name).execute().await.is_err() {
         ...
         self.db.open_table(&self.table_name).execute().await.map_err(|e| anyhow::anyhow!(e))
    }
