...
mod data;
mod database;
/// Hybrid AI Engine (Fast + Smart Tier)
...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route(
            "/api/ai/refactor/stream",
            post(handlers::ai_refactor_stream),
        )
        .route(
            "/api/ai/refactor/report",
            post(handlers::ai_refactor_report),
        )
        .route("/api/ai/explain", post(handlers::ai_explain))
        .route("/api/ai/models", get(handlers::list_ai_models))
        .route(
            "/api/ai/vector/search",
            post(ai_engine::handlers::search_vectors),
        )
        .route(
            "/api/ai/vector/index",
        ...
        .route(
            "/api/dataframe/schema",
            post(handlers::get_dataframe_schema),
        )
        ...
        .route(
            "/api/tasks/:id/branch",
            post(crate::tasks::create_branch_for_task),
        )
    ...
    tokio::spawn(async move {
        ...
        let result =
            tokio::task::spawn_blocking(|| crate::ai_engine::fast_tier::FastTierEngine::new())
                .await;
        ...
        match result {
            ...
            Ok(Err(e)) => {
                info!(
                    "Hybrid AI Engine initialization failed (likely download error): {}",
                    e
                );
            }
            Err(e) => {
    ...
    let app = app // Continue chain from the first definition
        ...
        .route(
            "/api/dotnet/add-package",
            post(handlers::dotnet_add_package),
        )
        ...
        .route(
            "/api/ml/training/progress/:session_id",
            get(handlers::ml_training_progress),
        )
        .route(
            "/api/ml/training/stop/:session_id",
            post(handlers::ml_stop_training),
        )
        ...
        .route(
            ...
            post(handlers::agent_create_session),
        )
        .route(
            "/api/agent/session/:session_id",
            get(handlers::agent_get_session),
        )
        .route(
            "/api/agent/context/:session_id",
            get(handlers::agent_get_context),
        )
        .route(
        ...
        .route(
            ...
            post(handlers::agent_enhanced_chat),
        )
        .route(
            "/api/agent/enhanced-chat/stream",
            post(handlers::agent_enhanced_chat_stream),
        )
        .route("/api/agent/index/start", post(handlers::agent_index_start))
        .route(
            "/api/agent/pending-changes/:session_id",
            get(handlers::agent_get_pending_changes),
        )
        .route(
            "/api/agent/apply-change/:change_id",
            post(handlers::agent_apply_change),
        )
        .route(
        ...
        .route(
            "/api/scenario/node/execute",
            post(handlers::scenario_execute_node),
        )
        ...
        .route(
            "/api/preview/agent/start",
            post(handlers::preview_start_agent),
        )
        .route(
        ...
        .route(
            ...
            get(handlers::hotreload_list_thunks),
        )
        .route(
            "/api/hotreload/thunk/register",
            post(handlers::hotreload_register_thunk),
        )
        .route(
            "/api/hotreload/state/snapshot",
            get(handlers::hotreload_state_snapshot),
        )
        .route(
            "/api/hotreload/state/clear",
            post(handlers::hotreload_state_clear),
        )
        .route(
            "/api/hotreload/rollback",
        ...
        .route(
            "/api/ai/complete",
            post(ai_engine::handlers::handle_completion),
        )
        // Network Proxy
        ...
        .route(
            "/api/pipeline/list",
            post(handlers::pipeline_list_workflows),
        )
        .route(
            "/api/pipeline/parse",
            post(handlers::pipeline_parse_workflow),
        )
        .route("/api/pipeline/save", post(handlers::pipeline_save_workflow))
        .route(
            "/api/pipeline/delete",
            post(handlers::pipeline_delete_workflow),
        )
        .route(
            "/api/pipeline/content",
            post(handlers::pipeline_get_content),
        )
        ...
        .route(
            "/api/pipeline/set-project",
            post(handlers::pipeline_set_project),
        )
        .route("/ws/pipeline/logs", get(handlers::pipeline_logs_ws))
        // Offline Agent
        .route("/api/agent/offline/status", get(handlers::offline_status))
        .route(
            "/api/agent/offline/install",
            post(handlers::offline_install),
        )
...
async fn handle_socket(mut socket: axum::extract::ws::WebSocket, state: Arc<RwLock<AppState>>) {
...
async fn handle_ws_message(message: &str, state: &Arc<RwLock<AppState>>) -> Result<String> {
...
mod handlers {
    ...
    pub async fn dotnet_version() -> impl IntoResponse {
        ...
        match manager.version().await {
            ...
            Err(e) => (
                ...
                format!("Failed to get .NET version: {}", e),
            )
                .into_response(),
        }
    }
    ...
    pub async fn dotnet_new_project(Json(payload): Json<NewProjectRequest>) -> impl IntoResponse {
        let manager = DotnetManager::new();
        match manager
            .new_project(&payload.template, &payload.name, &payload.output_dir)
            .await
        {
            Ok(output) => Json(json!({
    ...
    pub async fn dotnet_build(Json(payload): Json<BuildRequest>) -> impl IntoResponse {
        let manager = DotnetManager::new();
        match manager
            .build(&payload.project_path, &payload.configuration)
            .await
        {
            Ok(output) => Json(json!({
                "success": output.success,
    ...
    pub async fn ml_export_model(Json(payload): Json<ExportModelRequest>) -> impl IntoResponse {
        match crate::ml::export_model(&payload.session_id, &payload.format, &payload.output_path)
            .await
        {
    ...
    pub async fn ai_format(Json(payload): Json<AIFormatRequest>) -> impl IntoResponse {
        match crate::code::formatter::format_with_ai(
            &payload.code,
            &payload.language,
    ...
    pub async fn project_detect(Json(payload): Json<ProjectRequest>) -> impl IntoResponse {
        ...
        let supported: Vec<&str> = match project_type {
            ...
            crate::project::ProjectType::Flutter => {
                vec!["build", "run", "test", "clean", "install"]
            }
            ...
            crate::project::ProjectType::Android | crate::project::ProjectType::Kotlin => {
                vec!["build", "run", "test", "clean"]
            }
    ...
    pub async fn project_build(Json(payload): Json<ProjectRequest>) -> impl IntoResponse {
        use crate::project::{BuildAction, BuildConfig, ProjectBuilder};
        use std::path::Path;

        let config = BuildConfig {
            is_debug: payload.is_debug.unwrap_or(true),
        };
        let result =
            ProjectBuilder::execute(Path::new(&payload.path), BuildAction::Build, config).await;
    ...
    pub async fn project_run(Json(payload): Json<ProjectRequest>) -> impl IntoResponse {
        ...
        let result =
            ProjectBuilder::execute(Path::new(&payload.path), BuildAction::Run, config).await;
    ...
    pub async fn project_test(Json(payload): Json<ProjectRequest>) -> impl IntoResponse {
        ...
        let result =
            ProjectBuilder::execute(Path::new(&payload.path), BuildAction::Test, config).await;
    ...
    pub async fn project_clean(Json(payload): Json<ProjectRequest>) -> impl IntoResponse {
        ...
        let config = BuildConfig { is_debug: true };
        let result =
            ProjectBuilder::execute(Path::new(&payload.path), BuildAction::Clean, config).await;
    ...
    pub async fn agent_index_start(
        ...
        Ok(Json(
            serde_json::json!({ "success": true, "message": "Indexing started in background" }),
        ))
    }
    ...
    pub async fn agent_enhanced_chat_stream(
        ...
        Sse::new(stream.map(|res| {
            match res {
                Ok(event) => Event::default().json_data(event),
                Err(e) => Event::default()
                    .json_data(serde_json::json!({"type": "error", "data": e.to_string()})),
            }
        }))
    ...
    pub async fn db_connect(Json(payload): Json<DbConnectRequest>) -> impl IntoResponse {
        let db_type = match payload.db_type.as_str() {
            ...
            _ => {
                return (axum::http::StatusCode::BAD_REQUEST, "Invalid database type")
                    .into_response()
            }
        };

        match DB_MANAGER
            .connect(db_type, &payload.connection_string)
            .await
        {
    ...
    pub async fn db_query(Json(payload): Json<DbQueryRequest>) -> impl IntoResponse {
        match DB_MANAGER
            .execute_query(&payload.connection_id, &payload.query)
            .await
        {
            Ok(result) => Json(json!(result)).into_response(),
            Err(e) => {
    ...
    pub async fn proxy_request(
        Json(payload): Json<crate::network::ProxyRequest>,
    ) -> impl IntoResponse {
    ...
    pub async fn ai_transpile(
        ...
        let transpiled =
            crate::ai::transpile(&req.code, &req.target_language, &req.instructions, &config)
                .await?;
    ...
    pub async fn ai_refactor_report(Json(req): Json<AIRefactorReportRequest>) -> impl IntoResponse {
        match crate::ai_report::generate_pdf_report(req).await {
            Ok(pdf_bytes) => axum::response::Response::builder()
                .header("Content-Type", "application/pdf")
                .header(
                    "Content-Disposition",
                    "attachment; filename=\"refactor_report.pdf\"",
                )
            ...
            Ok(stream) => {
                let stream = stream.map(
                    |result: std::result::Result<
                        crate::runner::StreamEvent,
                        crate::error::AppError,
                    >|
                     -> std::result::Result<Event, Infallible> {
                        match result {
                            Ok(event) => Ok(Event::default().json_data(event).unwrap()),
                            Err(e) => Ok(Event::default().event("error").data(e.to_string())),
                        }
                    },
                );
    ...
    pub async fn scenario_execute(Json(req): Json<ScenarioExecuteRequest>) -> impl IntoResponse {
    ...
    pub async fn scenario_execute_node(Json(req): Json<NodeExecuteRequest>) -> impl IntoResponse {
        let mut executor = crate::scenario::ScenarioExecutor::new();
    ...
    pub async fn preview_reload(Json(req): Json<PreviewReloadRequest>) -> impl IntoResponse {
    ...
    pub async fn cicd_list_runs(
        ...
        match client
            .list_runs(&payload.owner, &payload.repo, &payload.token, 20)
            .await
        {
    ...
    pub async fn cicd_run_jobs(Json(payload): Json<RunJobsRequest>) -> impl IntoResponse {
        let client = crate::cicd::GitHubClient::new();
        match client
            .list_jobs(
                &payload.owner,
    ...
    pub async fn cicd_trigger(
        ...
        match client.trigger_workflow(&payload).await {
            Ok(_) => {
                Json(json!({ "success": true, "message": "Workflow triggered" })).into_response()
            }
            Err(e) => {
                (axum::http::StatusCode::INTERNAL_SERVER_ERROR, e.to_string()).into_response()
            }
    ...
    pub async fn pipeline_set_project(
        ...
        let st = state.read().await;
        st.pipeline_engine
            .set_project_dir(std::path::PathBuf::from(&payload.project_path))
            .await;
    ...
    pub async fn pipeline_save_workflow(
        ...
        match st
            .pipeline_engine
            .save_workflow(&payload.filename, &payload.content)
            .await
        {
            Ok(_) => Json(json!({ "success": true })).into_response(),
            Err(e) => (StatusCode::BAD_REQUEST, e).into_response(),
    ...
    pub async fn pipeline_get_content(
        ...
        match st
            .pipeline_engine
            .get_workflow_content(&payload.filename)
            .await
        {
    ...
    pub async fn pipeline_trigger(
        ...
        match st
            .pipeline_engine
            .trigger(&payload.workflow_file, payload.env_overrides)
            .await
        {
    ...
    pub async fn data_list(State(state): State<Arc<RwLock<AppState>>>) -> impl IntoResponse {
        let st = state.read().await;
                    ...
                    Message::Text(text) => {
                        ...
                        if text.starts_with("R:") {
                            ...
                            if parts.len() == 2 {
                                ...
                                {
                                    let _ =
                                        st.terminal_manager.resize(&recv_session_id, cols, rows);
                                }
                            }
                        } else {
    ...
    pub async fn offline_status() -> impl IntoResponse {
        ...
        match client.get("http://127.0.0.1:11434/api/tags").send().await {
            ...
            _ => Json(serde_json::json!({ "status": "stopped", "models": [] })),
        }
    }
    ...
    pub async fn offline_install(Json(req): Json<OfflineInstallReq>) -> impl IntoResponse {
            ...
            .await
        {
            Ok(res) if res.status().is_success() => Json(
                serde_json::json!({ "success": true, "message": format!("Successfully installed {}", req.model) }),
            ),
            Ok(res) => {
                let err = res.text().await.unwrap_or_default();
                Json(
                    serde_json::json!({ "success": false, "error": format!("Ollama returned error: {}", err) }),
                )
            }
            Err(e) => Json(
                serde_json::json!({ "success": false, "error": format!("Failed to connect to Ollama: {}", e) }),
            ),
        }
    }
}
