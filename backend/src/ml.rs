...
pub async fn scan_dataset(path: &str) -> Result<MLDataset, String> {
    ...
    while let Some(entry) = entries.next_entry().await.map_err(|e| e.to_string())? {
        ...
        if entry.path().is_dir() {
            match file_name.as_str() {
                "train" | "training" => {
                    train_path = Some(entry.path().to_string_lossy().to_string())
                }
                "val" | "validation" => {
                    validation_path = Some(entry.path().to_string_lossy().to_string())
                }
    ...
    while let Some(entry) = class_entries
        .next_entry()
        .await
...
async fn run_training_loop(
    ...
    let mut weights: Vec<f32> = (0..weight_size)
    ...
    for epoch in 1..=total_epochs {
        ...
        let sleep_ms = if total_epochs > 50 {
            5
        } else if total_epochs > 20 {
            15
        } else {
            30
        };
        ...
        "warmup_cosine" => {
            ...
            if epoch <= warmup_epochs {
                ...
                let adjusted_progress =
                    (epoch - warmup_epochs) as f64 / (total - warmup_epochs) as f64;
...
async fn export_coreml(model: &TrainedModel, output_path: &str) -> Result<String, String> {
    ...
    let mut file = fs::File::create(&spec_path)
        .await
        .map_err(|e| e.to_string())?;
    file.write_all(serde_json::to_string_pretty(&spec).unwrap().as_bytes())
        .await
        .map_err(|e| e.to_string())?;

    // Write binary weights
    let weights_path = bundle_path.join("weights.bin");
    let weights_bytes: Vec<u8> = model.weights.iter().flat_map(|f| f.to_le_bytes()).collect();
...
async fn export_mlpackage(model: &TrainedModel, output_path: &str) -> Result<String, String> {
    ...
    fs::create_dir_all(&bundle_path)
        .await
        .map_err(|e| e.to_string())?;
    fs::create_dir_all(bundle_path.join("Data/com.apple.CoreML"))
        .await
        .map_err(|e| e.to_string())?;
    ...
    let weights_bytes: Vec<u8> = model.weights.iter().flat_map(|f| f.to_le_bytes()).collect();
    fs::write(
        bundle_path.join("Data/com.apple.CoreML/weights.bin"),
        weights_bytes,
    )
    .await
    .map_err(|e| e.to_string())?;
...
async fn export_onnx(
    ...
    fs::write(&onnx_path, data)
        .await
        .map_err(|e| e.to_string())?;

    // Labels file
    let labels = model.classes.join("\n");
    fs::write(onnx_path.with_extension("labels.txt"), labels)
        .await
        .map_err(|e| e.to_string())?;
...
async fn export_saved_model(model: &TrainedModel, output_path: &str) -> Result<String, String> {
    ...
    fs::create_dir_all(&saved_model_dir)
        .await
        .map_err(|e| e.to_string())?;
    fs::create_dir_all(saved_model_dir.join("variables"))
        .await
        .map_err(|e| e.to_string())?;
    fs::create_dir_all(saved_model_dir.join("assets"))
        .await
        .map_err(|e| e.to_string())?;
    ...
    fs::write(saved_model_dir.join("saved_model.pb"), pb_data)
        .await
        .map_err(|e| e.to_string())?;
    ...
    let weights_bytes: Vec<u8> = model.weights.iter().flat_map(|f| f.to_le_bytes()).collect();
    fs::write(
        saved_model_dir.join("variables/variables.data-00000-of-00001"),
        weights_bytes,
    )
    .await
    .map_err(|e| e.to_string())?;
    ...
    // Labels
    fs::write(
        saved_model_dir.join("assets/labels.txt"),
        model.classes.join("\n"),
    )
    .await
    .map_err(|e| e.to_string())?;
